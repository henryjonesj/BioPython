...
def alignGlobaly(seq1,seq2,matrix):
	...
	for i in range(0, 2):
		for j in range(0, 2):
			...
			alignments = pairwise2.align.globalds(seq1.seq, seq2.seq, matrix, gapOpenPenalty[i], gapExtendPenalty[j])
			computeParameters(alignments)


def doAlignment(matrix):

	print "\n Computing for Brisbane2007.txt and California2009.txt……."

	alignGlobaly(seq1,seq2,matrix);

	print "\n Computing for California2009.txt and SC1918.txt..……."

	alignGlobaly(seq2,seq3,matrix);

	print "\n Computing for Brisbane2007.txt and SC1918.txt..……."

	alignGlobaly(seq1,seq3,matrix);
...
seq2 = SeqIO.read("California2009.txt", "fasta")
seq3=  SeqIO.read("SC1918.txt", "fasta")
